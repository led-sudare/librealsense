...
{
    if (major(version) == 0)
        return rsimpl::to_string() << version;
...
rs_context *rs_create_context(int api_version, rs_error **error) try
{
    ...
    if (local_error)
        throw std::runtime_error(rs_get_error_message(local_error));
    ...
    else if ((major(runtime_api_version) == 1 && minor(runtime_api_version) <= 9) || (major(api_version) == 1 && minor(api_version) <= 9))
    {
    ...
    else
    {
        ...
        if ((major(api_version) != major(runtime_api_version)) || (minor(api_version) != minor(runtime_api_version)))
            report_version_mismatch(runtime_api_version, api_version);
    }
