        ...
        for (auto pad_crop : subdevice_mode.pad_crop_options)
        {
            for (auto &unpacker : subdevice_mode.pf.unpackers)
            {
                ...
                for (auto &output : unpacker.outputs)
                {
                    ...
                    if (req.enabled && (req.width == selection.get_width()) && (req.height == selection.get_height()) && (req.format == selection.get_format(output.first)) && (req.fps == subdevice_mode.fps))
                    {
    ...
    for (auto &rule : info.interstream_rules)
    {
        ...
        if (a.enabled && b.enabled)
        {
            ...
            if (rule.same_format)
            {
                ...
                {
                    if (throw_exception)
                        error_message << rule.a << " format (" << rs_format_to_string(a.format) << ") must be equal to " << rule.b << " format (" << rs_format_to_string(b.format) << ")!";
                    compat = false;
                }
            }
            else if ((a.*f != 0) && (b.*f != 0))
            {
                if ((rule.bigger == RS_STREAM_COUNT) && (!rule.divides && !rule.divides2))
                {
                    ...
                    {
                        if (throw_exception)
                            error_message << " " << rule.b << " value " << b.*f << " must be equal to either " << (a.*f + rule.delta) << " or " << (a.*f + rule.delta2) << "!";
                        compat = false;
                    }
                }
                else
                {
                    ...
                    {
                        if (throw_exception)
                            error_message << " " << rule.a << " value " << a.*f << " must be " << ((rule.bigger == rule.a) ? "bigger" : "smaller") << " then " << rule.b << " value " << b.*f << "!";
                        compat = false;
                    }
                    if ((rule.divides && (a.*f % b.*f)) || (rule.divides2 && (b.*f % a.*f)))
                    {
                        if (throw_exception)
                            error_message << " " << rule.a << " value " << a.*f << " must " << (rule.divides ? "be divided by" : "divide") << rule.b << " value " << b.*f << "!";
                        compat = false;
                    }
                }
            }
